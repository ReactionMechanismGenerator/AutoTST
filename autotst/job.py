...
class Job():
    ...
    def submit(calculator, partition):
        ...
        complete_file_path = file_path.replace("left", "(").replace("right", ")")
        ...
        if os.path.exists(complete_file_path + ".log"):
            ...
            try:
                ...
                subprocess.call("sbatch --exclude=c5003 --job-name={0} --output={0}.slurm.log --error={0}.slurm.log -p {1} -N 1 -n 20 --mem=100000 submit.sh".format(calculator.label, partition), shell=True)
    ...
    def submit_species(self, species=None, calculator=None):
        ...
        for smiles, confs in species.conformers.items():
            for conf in confs:
                calc = self.submit_conformer(conformer=conf, calculator=calculator)
                calcs.append(calc)  
        return calcs
    ...
    def calculate_species(self, species=None, calculator=None):
        ...
        species.generate_conformers(calculator=Hotbit()) ### This doesn't work rn because RDKit is acting up on discovery
                    ...
                    if check_complete(ase_calculator=calc):
                        if not first_try[calc.label]:
                            ...
                            try: #reading in results after the first attempt
                                conf.ase_molecule = self.read_log(os.path.join(calc.scratch, calc.label + ".log"))
                                ...
                                logging.info("{} failed, trying it again...".format(calc.label))
                                submit_conformer(conformer=conf, calculator=calculator)
                                first_try[calc.label] = True
                            ...
                            try:
                                ...
                                logging.info("{} failed a second time...".format(calc.label))
                                complete[calc.label] = True
    ...
    def submit_ts(self, ts=None, calculator=None, vibrational_analysis=True):
        ...
        try:
            ...
            try:
                ts.ase_molecule = self.read_log(os.path.join(
                    calc.scratch,
                    calc.label.replace("left", "(").replace("right", ")") + ".log"
                ))
                ts.update_coords()
                logging.info("{} was successful!".format(calc.label))
                calculator.fix_io_file(calc)
        ...
        if vibrational_analysis: #If we're running vibrational analysis
            ...
            if not result: 
                logging.info("Vibrational analysis not definitive. Running IRC instead")
